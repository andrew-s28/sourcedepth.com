...
As I've said, posts on this site are built on MDX. However, I still needed to design custom components. One major component I needed to include was support for time series plots to showcase data (I am a scientist, after all), which ended up being quite the rabbit hole. I built my plots using [Recharts](https://recharts.org/), which is a React library built on D3.js.

I needed support for a variable time series axis with user defined calendar dates, which I had to implement. I built a custom date range picker based on [React DayPicker](https://daypicker.dev/) with theming inspired by [tremor](https://tremor.so/docs/inputs/date-range-picker). I couldn't use tremor's solution out of the box since it didn't support React DayPicker 9, so I built my solution but used their theming.

I also wanted my plots to support dark mode through Tailwind classes, since state does not seem to update when using MDX bundler. This required using custom legend entries, tooltips, and line colors since Recharts usually uses the style from the parent element, which I wasn't providing (using Tailwind classes instead).
